...
fn delay_s(comptime _cctx: used_mcu.ctx.Ct_Context, _rctx: *used_mcu.ctx.Rt_Context, _s: usize) void {
    _ = _cctx;
    _ = _rctx;
    const begin = @atomicLoad(usize, &s_count, .acquire);
    while (begin + _s > @atomicLoad(usize, &s_count, .acquire)) {}
}
...
fn main_with_err(comptime _cctx: used_mcu.ctx.Ct_Context) !void {
    ...
    const wwdg = used_mcu.wdg.wwdg;
    const c13 = used_mcu.gpio.C.pin(.@"13");

    wwdg.it_start(cctx, rctx);
    wwdg.start(cctx, rctx);
...
fn rtc_global_it_gf(comptime _cctx: used_mcu.ctx.Ct_Context) fn () void {
    ...
    return struct {
        pub fn f() void {
            ...
            _ = @atomicRmw(usize, &s_count, .Add, 1, .release);
        }
    }.f;
}

fn wwdg_it_gf(comptime _cctx: used_mcu.ctx.Ct_Context) fn () void {
    const wwdg = used_mcu.wdg.wwdg;
    return struct {
        pub fn f() void {
            defer wwdg.clear_it_flag(_cctx, &rt_ctx);
            wwdg.feed(_cctx, &rt_ctx);
        }
    }.f;
}
